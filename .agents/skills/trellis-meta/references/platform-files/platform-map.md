...
| Snow CLI | `--snow` | `.snow/` | `.snow/skills/` | `.snow/agents/` (auto-discovered) | class-1 inject hooks: `.snow/hooks/` (`onSessionStart` / `onUserMessage` / `beforeSubAgentStart` + `additionalContext` JSON); commands `.snow/commands/trellis-*.json` |
...
- Snow CLI (`.snow/agents/` auto-discovered; class-1 inject hooks)
