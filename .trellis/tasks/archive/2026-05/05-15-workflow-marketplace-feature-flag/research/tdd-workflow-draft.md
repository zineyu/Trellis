...
Phase 2: Execute → implement only after task status is in_progress; use one red/green/refactor slice per behavior
...
**What to put in**:
- **Testing specs** — unit/integration test conventions and mock strategy files relevant to the task
...
Spawn the implement sub-agent with the TDD loop in the task description:

- **Agent type**: `trellis-implement`
- **Task description**: Implement reviewed task artifacts using one behavior slice at a time: red test through a public interface, green implementation, refactor only while green; finish by running focused tests plus project lint and type-check
